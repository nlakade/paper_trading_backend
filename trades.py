...
def create_trade():
    try:
        ...
        margin_required = current_price * data['quantity'] * 0.2
        
        if portfolio.get('available_margin', 0) < margin_required:
        ...
        if not update_portfolio_margin(client_id, margin_required):
            mongo.db.trades.delete_one({'_id': trade_result.inserted_id})
        ...
        send_notification_helper(
            client_id,
            'EMAIL',
            f"Trade created: {data['trade_type']} {data['quantity']} {data['symbol']} at {current_price}"
        )
...
def exit_trade(trade_id):
    try:
        ...
        update_portfolio_margin(user_id, -trade['margin_required'], pnl)
        ...
        logger.info(f"Trade exited: {trade_id} for user: {user_id}")
...
def exit_all_trades(email):
    try:
        ...
        active_trades_list = Trade.find_active_by_user_id(str(user_oid))  
        results = []
        total_pnl = 0
...
@trades_bp.route('/performance/<email>', methods=['GET'])  # email path param
@jwt_required()
def get_trade_performance(email):
    try:
        ...
        trades = Trade.find_by_user_id(str(user_oid))  
        
        total_trades = len(trades)
        active_trades_count = len(Trade.find_active_by_user_id(str(user_oid)))
        ...
        try:
            ...
            for trade_id, _ in active_trades.items():
                ...
                if stop_loss_hit or target_hit:
                    ...
                    user = User.find_by_id(str(trade['user_id']))
                    if user:
                        client_email = user.get('client_id') or user.get('email')  # Adjust based on your User model
                        update_portfolio_margin(client_email, -trade['margin_used'], pnl)
                        ...
                        logger.warning(f"User  not found for trade {trade_id} during auto-close")
